...
function checkIdLength(req: Request, res: Response, next: NextFunction) {
  ...
  if (elementId.length !== 36) {
    res.locals.validationError = 'wrong_id';
    next(new Error('Id should be 36 characters long'));
  }
  next();
}
...
function checkNameLength(req: Request, res: Response, next: NextFunction) {
  if (req.body.name.length < 3) {
    res.locals.validationError = 'wrong_name';
    next(new Error('Name should be 3 characters minimum'));
  }
  next();
}
